...
if($search):
    ...
    //client
    if($_REQUEST['client']) {
    //This is client based search..perm taken care above..put the sucker in.
        $qwhere.=' AND client.client_id='.db_input($_REQUEST['client']);
        $qstr.='&client='.urlencode($_REQUEST['client']);
    }
    //department
...
$qselect = 'SELECT DISTINCT ticket.ticket_id,lock_id,ticketID,ticket.dept_id,ticket.staff_id,subject,name,ticket.email,dept_name, concat(firstname," ", lastname) as assigned_name, client_name  '.
...
$qfrom=' FROM '.TICKET_TABLE.' ticket '.
       ...
       ' LEFT JOIN custom_client client ON ticket.client_id=client.client_id '.
       ' LEFT JOIN ost_staff staff2 ON ticket.staff_id=staff2.staff_id ';
 ...
 <input type="hidden" name="a" value="search">
  <table>
    <tr>
        ...
        <td>Client:</td>
        <td><select name="client"><option value=0>All Clients</option>
            <?
                //Showing only departments the user has access to...
                $sql='SELECT client_id,client_name FROM custom_client';
                
                $clients= db_query($sql);
                while (list($clientId,$clientName) = db_fetch_row($clients)){
                $selected = ($_GET['client']==$clientId)?'selected':''; ?>
    ...
    <tr><td>
       <table width="100%" border="0" cellspacing=0 cellpadding=2 class="dtable" align="center">
        <tr>
	        ...
	        <th width="120">
                <a href="tickets.php?sort=client&order=<?=$negorder?><?=$qstr?>" title="Sort By Client <?=$negorder?>">Client</a></th>
	        <th width="120">
        ...
        if($tickets_res && ($num=db_num_rows($tickets_res))):
            ...
            <tr class="<?=$class?> " id="<?=$row['ticket_id']?>">
                ...
                <td nowrap><?=Format::truncate($row['client_name'],30)?></td>
		...
		<?php }else{ ?>
                <td nowrap><?=Format::truncate($row['assigned_name'],22,strpos($row['assigned_name'],'@'))?>&nbsp;</td>
		<?php } ?>
            </tr>
            <?

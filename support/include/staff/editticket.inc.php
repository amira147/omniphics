...
<table width="100%" border="0" cellspacing=1 cellpadding=2>
  <form action="tickets.php?id=<?=$ticket->getId()?>" method="post">
    ...
    <?php
    $services= db_query('SELECT client_id,client_name FROM custom_client ORDER BY client_name');
    if($services && db_num_rows($services)){ ?>
    <tr>
        <td align="left" valign="top"><b>Client Company:</b></td>
        <td>
            <select name="clientId">    
                <option value="0" selected >None</option>
                <?if(!$info['clientId'] && $info['client_name']){ //old helptopic?>
                <option value="0" selected ><?=$info['client_name']?> (deleted)</option>
                <?
                }
                 while (list($clientId,$client) = db_fetch_row($services)){
                    $selected = ($info['clientId']==$clientId)?'selected':'';
                    ?>
                    <option value="<?=$clientId?>"<?=$selected?>><?=$client?></option>
                <?
                 }?>
            </select>
            &nbsp;<font class="error">*&nbsp;<?=$errors['clientId']?></font>
        </td>
    </tr>
    <?
    }?>
